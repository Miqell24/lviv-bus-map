...
// Lviv: ONE feed covering the whole city network — 55 bus lines run by seven
// private operators, the nine trolleybus lines of ЛКП Львівелектротранс in the
// family's green, and its eight tram lines in red.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|1,8]
...
// Exact-match repairs for poles a feed names badly. Lviv needs none: of 1 061
// stop names only seven shout, and they are acronyms.
const NAME_FIX = {};
...
const tramSel = tramLines.filter((l) => l !== 'all');
...
// route_type 3 — the Mexico City trap, where the feed calls a trolleybus a
// bus. Here the giveaway is doubled: the Тр prefix and the operator, ЛКП
// Львівелектротранс, which runs nothing else on the road.
//
// LINE KEYS. The prefixes are the feed's bookkeeping, not the street's: Lviv
// signs its vehicles "1", "22", "37" and says the mode in words. So the keys
// keep А/Т/Тр — they have to, since tram 7 and bus 7 both exist — and LBL
// prints the bare number ([[transit-maps-bare-numbers]], the Rybnik rule).
const LBL = new Map();
const isRailTrunk = () => false;

const TROLLEY_OP = 'ЛКП Львівелектротранс';
const bare = (sn) => {
  const s = (sn || '').trim();
  const m = /^(Тр|Т|А)\s*0*(\d.*)$/.exec(s);
  return m ? m[2] : s;
};
const lineKey = (sn) => {
  const k = (sn || '').trim();
...
const MODES = [{
  ...
  // the city and its ring of villages — 2 × 2 tiles
  osmFiles: Array.from({ length: 4 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['А01']),
  feeds: [
    { tag: 'lv', dir: 'data/gtfs', routeTypes: ['3'], mapKey: lineKey,
      // trolleybus, whatever the feed's route_type says
      trolley: (r) => /^Тр/.test((r.route_short_name || '').trim()) },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/lviv-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel.map((l) => (/^Т/.test(l) ? l : 'Т' + String(l).padStart(2, '0'))),
  feeds: [
    { tag: 'lv', dir: 'data/gtfs', routeTypes: ['0'], mapKey: lineKey },
  ],
});
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      } else if (['1', '2', '6'].includes(r.route_type) && /^[0-9A-F]{6}$/i.test(r.route_color || '')) {
        // Official line colours are for METRO and SUBURBAN RAIL only — route_type
        // 0 is deliberately not in that list. Lviv's feed paints its trams
        // #556B2F and #FFFFFF, and the family rule is that a tram is red in
        // every city; a white tram line on the paper base is also simply
        // invisible. There is no metro and no rail here, so nothing is left to
        // inherit a colour — the branch stays for the shape of the family.
